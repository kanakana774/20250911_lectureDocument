...
- オブジェクト指向の勘違い
  ...
  - オブジェクト指向の 3 要素
    - カプセル化（クラス）⇐ これについての勘違い
    - 継承
    - ポリモーフィズム
  - 勘違い１：粒度は目的によって変わる
    - 我々一般人は車は移動できれば目的を達成できるため、車という粒度で見ればよく、Car クラスと認識すればよい
    - 一方整備士などは整備することが目的なので、車はエンジンなど部品の集合と見ないと行けず、Engine クラスや Wheel クラスという粒度で見ないとならない
  - 勘違い２：現実のものと１：１の関係でクラスを作るとだいたい変更に弱くなる（変更に弱い＝ビジネスの変化についていけない＆メンテナンスコストが高い）
    - 飲食店を例に、注文をするというユースケースで考えると、ホール担当、キッチン担当、店長みたいな現実に沿ったクラスを作りがちだが、、、これは変更に弱いシステムになってしまう
      - 変更に強いとされるレイヤードアーキテクチャと比較してみると、例えば、ホール担当は客とのやり取りの他に、一部業務ルールの判断も兼任していて責務が分離されてない
    - 誤解を恐れずに言えば、アーキテクチャは変更に強くするための責務分離のテンプレといえる
    - もちろん、大枠としての分離のテンプレなので、特にアプリケーション層などはさらに変更理由毎に細分化される

- ただ、大前提、設計に関わることはまだ先のことなので今抑えておくのは下記で、アーキテクチャなどは下記を理解するための背景知識
  - 処理はしかるべき場所に書く（後の可読性のため）
  - コードを読む際にプロジェクトのどのクラスにその処理が書いてありそうか構造などを把握して見つける力を付ける
...
| 項目       | ウェイターの仕事 (Controller)                        | 注文処理サービスの仕事 (Service)                             |
| :--------- | :--------------------------------------------------- | :----------------------------------------------------------- |
